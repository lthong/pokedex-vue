  ...
  <div class="pokedex-detail">
    <template v-if="!loading && String(basicInfo.id) === id">
      ...
      <div :class="['pokemon', { [mainType]: !!mainType }]">
        <img key="prePokeDetailBtn" :class="['arrow-btn left', { disabled: preBtnDisabled }]" :src="assets.arrowIcon"
          alt="arrow-icon" @click="onArrowBtnClick('pre')" />
        <div class="data-box">
          <div class="block">
            <SaveMyPokeBtn :id="id" />
            <div class="img-container">
              <img class="poke-photo" :src="`https://assets.pokemon.com/assets/cms2/img/pokedex/full/${formatPokeId}.png`"
                :alt="name" />
            </div>
          </div>
          <div class="row">
            <div class="col-1-1">
              ...
              <div class="content">
                <div v-for="({ label, value }, index) in states" :key="`state-${index}`" class="state">
                  <div class="label">{{ $t(`common.${label}`) }}</div>
                  <div class="value">
                    <div :class="['bar', { full: getBarWidth(value) === 100 }]" :style="isStatesBarReady
        ...
        <div class="data-box">
          ...
          <div class="row">
            <div class="col-1-1">
              ...
              <div class="content">
                <PokemonType v-for="(item) in basicInfo.types" :key="item.type.name" :type="item.type.name" />
              </div>
            </div>
          </div>
          <div class="row">
        ...
        <img key="nextPokeDetailBtn" :class="['arrow-btn right', { disabled: nextBtnDisabled }]" :src="assets.arrowIcon"
          alt="arrow-icon" @click="onArrowBtnClick('next')" />
      </div>
    </template>
